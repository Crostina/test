...
$roomC = new roomC();
$availableRoom = $roomC->getAvailableRoom();
$idS = 1;
if ($availableRoom) {
    $idS = $availableRoom['idS'];
    $APP_ID = $availableRoom['APP_ID'];
    $TOKEN = $availableRoom['TOKEN'];
    $CHANNEL = $availableRoom['CHANNEL'];
} else {
    // Handle the case when no room is available
    $appID = ""; // Provide a default value or handle it accordingly
    $token = "";
    $channel = "";
}
...
if (isset($_GET['idSC'])) {
    ...
    echo "Name parameter not set in the URL<br>";
}
...
<a href="index.php?idS=<?= $idS ?>&amp;APP_ID=<?= rawurlencode($APP_ID) ?>&amp;TOKEN=<?= rawurlencode($TOKEN) ?>&amp;CHANNEL=<?= rawurlencode($CHANNEL) ?>">join your session now</a>
